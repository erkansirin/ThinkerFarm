...
class ModelDataHandler: NSObject {
    ...
    var threshold: Float = ModelSettings.threshold
    
    // MARK: Model parameters
    let batchSize = ModelSettings.batchSize
    let inputChannels = ModelSettings.inputChannels
    let inputWidth = ModelSettings.inputWidth
    let inputHeight = ModelSettings.inputHeight
    ...
    init?(modelFileInfo: FileInfo, labelsFileInfo: FileInfo, threadCount: Int = 1) {
        ...
        if let path = Bundle.main.path(forResource: modelFilename, ofType: "tflite") {
            ...
            let fileURL = dir?.appendingPathComponent(modelFilename).appendingPathExtension(modelFileInfo.extension)
            ...
            do {
                // Create the `Interpreter`.
                interpreter = try Interpreter(modelPath: fileURL!.absoluteString, options: options)
                // Allocate memory for the model's input `Tensor`s.
                try interpreter.allocateTensors()
            } catch let error {
                print("Failed to create the interpreter with error: \(error.localizedDescription)")
            ...
            // Load the classes listed in the labels file.
            loadLabelsDoc(fileInfo: labelsFileInfo)
    ...
    /// Loads the labels from the labels file and stores them in the `labels` property.
    private func loadLabelsDoc(fileInfo: FileInfo) {
        let filename = fileInfo.name
        let fileExtension = fileInfo.extension
        let dir = try? FileManager.default.url(for: .documentDirectory,
                                               in: .userDomainMask, appropriateFor: nil, create: true)
